...
const AllProducts = () => {
  ...
  const handleSorting = (sortValue) => {
    sortValue
      ? setSortedValue(sortValue) || handleActive(sortValue)
      : setSortedValue(null) || handleActive(null);
  };
  ...
  const applyFilters = () => {
    ...
    // Filtering
    const checkedItems = updatedFilterMenu
      .map((item) => {
        return {
          ...item,
          menu: item.menu
    ...
    if (checkedItems.length) {
      updatedProducts = updatedProducts.filter(
        (item) =>
          checkedItems.includes(item.brand.toLowerCase()) ||
          checkedItems.includes(item.category.toLowerCase())
      );
    }
